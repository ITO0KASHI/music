<template>
  <div class="play-bar" :class="{ show: !toggle }">
    <div class="fold" :class="{ turn: toggle }">
      <music-icon :icon="iconList.ZHEDIE" @click="toggle = !toggle"></music-icon>
    </div>
    <!--播放进度-->
    ...
    <div class="control-box">
      ...
      <div class="song-ctr">
        <music-icon class="music-play-show" :icon="playStateList[playStateIndex]" @click="changePlayState"></music-icon>
        <!--上一首-->
        <music-icon class="music-play-show" :icon="iconList.SHANGYISHOU" @click="prev"></music-icon>
        <!--播放-->
        <music-icon :icon="playBtnIcon" @click="togglePlay"></music-icon>
        <!--下一首-->
        <music-icon class="music-play-show" :icon="iconList.XIAYISHOU" @click="next"></music-icon>
        <!--音量-->
        <el-dropdown class="music-play-show" trigger="click">
          <music-icon v-if="volume !== 0" :icon="iconList.YINLIANG"></music-icon>
          <music-icon v-else :icon="iconList.JINGYIN"></music-icon>
          <template #dropdown>
            <el-dropdown-menu>
              <el-slider class="music-slider" style="height: 150px; margin: 10px 0" v-model="volume" :vertical="true"></el-slider>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
      <div class="song-ctr song-edit">
        ...
        ></music-icon>
        <!--歌曲列表-->
        <music-icon :icon="iconList.LIEBIAO" @click="changeAside"></music-icon>
      </div>
    </div>
  </div>
</template>
...
@import "@/assets/css/music-play-bar.scss";
</style>
